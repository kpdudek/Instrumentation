...
avgData = mean(data);
stdData = std(data);
fprintf('Mean Volt: %5.3f\n',avgData)
fprintf('STD Volt: %5.3e\n',stdData)
% Lab specific values
...
plot(temp,avgData,'.','MarkerSize',14)
...
disp('Calcualted Resistance')
disp(Rtherm)
disp('Measured Resistance')
disp(Rmeas)

% Plot calculated and measured resistance vs temp
figure('Name','Thermistor Resistance')
plot(temp,Rtherm,'.','MarkerSize',14)
hold on
plot(temp,Rmeas,'.','MarkerSize',14)
xlabel('Temperature ($^{\circ}C)$','Interpreter','latex')
...
plot(lnR,invT,'.','MarkerSize',14)
xlabel('$ln(R)$','Interpreter','latex')
ylabel('$\frac{1}{T}(K)$','Interpreter','latex')
...
uncert_T = sqrt((U_volt/coeff(1)).^2 + U_temp^2);
uncert_V = sqrt((U_volt).^2 + (U_temp*coeff(1))^2);

fprintf('Up Fit = %5.3e\n',Up_fit)
fprintf('Up Mean = %5.3e\n',Up_mean)
fprintf('Total U Voltage = %5.3e\n',U_volt)
fprintf('Total U Temperature = %5.3e\n',U_temp)
fprintf('Temp Uncertainty = %5.3f\n',uncert_T)
fprintf('Volt Uncertainty = %5.3e\n',uncert_V)

% Plot calibration
figure('Name',iden)
plot(temp,volt,'.','MarkerSize',14)
hold on
plot(temp,volt_fit,'r','LineWidth',1.5)
